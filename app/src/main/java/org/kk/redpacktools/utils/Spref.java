package org.kk.redpacktools.utils;
...
import org.kk.redpacktools.BuildConfig;
...
public class Spref {
    ...
    public Spref(Context context) {
        ...
        mSpref = context.getSharedPreferences(getConfigFileName(), Context.MODE_PRIVATE);
        mEditor = mSpref.edit();
    }

    public Spref(Context context, String sprefName) {
        if (context == null) {
            Logger.e("Init Failed, context is null!");
            return;
        }
    ...
    public Spref(Context context, String sprefName, int mode) {
        if (context == null) {
            Logger.e("Init Failed, context is null!");
            return;
        }
    ...
    private String getConfigFileName() {
        return BuildConfig.APPLICATION_ID + "_prefs";
    }
     ...
     * @return 引用的KV对象。
     */
    public Spref put(String key, Object value) {
     ...
     * @return 当且仅当提交成功时返回true, 否则返回false.
     */
    public Spref put(Map<String, ?> keyValueMap) {
     ...
     * @return 引用的KV对象。
     */
    public Spref remove(String key) {
        if (!initSuccess()) {
            return this;
        }
        mEditor.remove(key);
